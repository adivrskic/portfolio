  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (activeBullet) {
        ...
        const index = Array.from(activeBullet.parentElement.children).indexOf(
          activeBullet
        );
    ...
    <>
      ...
      <div className="content">
            ...
            <div className="swiper-wrapper">
              {data?.map((item, i) => {
                ...
                const boxShadow = isActive
                  ? `
                    inset 0 0 25px ${
                      item.color1 || "rgba(255, 255, 255, 0.25)"
                    }, 
                    inset 10px 0 40px ${
                      item.color2 || "rgba(255, 0, 255, 0.25)"
                    }, 
                    inset -10px 0 40px ${
                      item.color3 || "rgba(0, 255, 255, 0.25)"
                    }, 
                    inset 10px 0 150px ${
                      item.color2 || "rgba(255, 0, 255, 0.25)"
                    }, 
        ...
        <div className="slide-info">
          {data[activeSlide] && (
            <a
              className="slide-info-link"
              href={data[activeSlide]?.link}
              target="_blank"
              rel="noreferrer"
            >
          ...
          <div className="slide-info-skills">
            {data[activeSlide] &&
              data[activeSlide]?.skills?.map((skill) => (
